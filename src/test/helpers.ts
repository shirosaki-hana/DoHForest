 ...
 */
export function queryUdp(testCase: TestCase, id?: number): Promise<DecodedResponse> {
 ...
 */
export function queryTcp(testCase: TestCase, id?: number): Promise<DecodedResponse> {
  return new Promise((resolve, reject) => {
 ...
 */
export function sendRawUdp(buf: Buffer, timeoutMs = 2000): Promise<Buffer | null> {
  return new Promise((resolve, reject) => {
    const client = dgram.createSocket('udp4');
 ...
 */
export function sendTcpPipelined(messages: Buffer[], expectedResponses: number): Promise<DecodedResponse[]> {
  return new Promise((resolve, reject) => {
    const client = new net.Socket();
    ...
    const timer = setTimeout(() => {
      client.destroy();
      reject(new Error(`TCP pipelining timeout: got ${results.length}/${expectedResponses}`));
    }, TIMEOUT_MS);
 ...
 * 전송 후 잠시 대기한 뒤 연결 종료
 */
export function sendRawTcp(buf: Buffer, timeoutMs = 2000): Promise<Buffer | null> {
...
export function getNonOptAnswers(response: DecodedResponse): Exclude<Answer, dnsPacket.OptAnswer>[] {
  return (response.answers ?? []).filter((a): a is Exclude<Answer, dnsPacket.OptAnswer> => a.type !== 'OPT');
}

export function encodeDnsQuery(testCase: TestCase, id?: number, additionals?: dnsPacket.OptAnswer[]): Buffer {
